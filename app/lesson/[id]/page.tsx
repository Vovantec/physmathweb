      ...
      <div className="max-w-4xl mx-auto">
        ...
        {deadlineInfo && (
          <DeadlineBanner
            ...
            isLate={deadlineInfo.isLate}
            isBlocked={deadlineInfo.isBlocked}
            policy={lesson.task.course.deadlinePolicy}
            penaltyMultiplier={deadlineInfo.penaltyMultiplier}
          />
        )}
          ...
          >
            <div className="flex justify-between items-start md:items-center flex-col md:flex-row gap-4">
              <div>
                ...
                {result.isLate && !result.isHistory && (
                  <p className="text-sm text-orange-400 font-mono mt-1">
                    ⏰ Сдано с опозданием
                    {result.penaltyApplied &&
                      ` — применён штраф ${Math.round(result.penaltyMultiplier * 100)}%`}
                  </p>
                )}
              </div>
            ...
            <div className="relative aspect-video bg-black border-2 border-white/20 rounded-xl flex items-center justify-center overflow-hidden shadow-2xl mt-4">
              {lesson.questions[selectedQuestionIndex]?.videoUrl ? (
                ...
                <div className="text-gray-500 font-mono text-center px-4">
                  Видеоразбор для этой задачи еще не добавлен.
                </div>
              )}
            </div>
          </div>
          ...
          {!canSolve || (result && !result.isHistory) ? (
            <div className="text-center py-8 opacity-50">
              <p className="text-gray-400 font-mono text-lg">
                {deadlineInfo?.isBlocked
                  ? 'Сдача заблокирована — дедлайн истёк.'
                  : bestPercent === 100
                  ? 'Вы успешно завершили это задание.'
                  : attemptsCount >= maxAttempts
                  ? 'Вы использовали все попытки.'
                  : 'Для повторного решения нажмите «Попробовать еще раз».'}
              </p>
            </div>
          ) : (
              ...
              {lesson.questions.length > 0 && (
                <div className="pt-8 border-t border-white/10 flex justify-end">
                  <button
                    ...
                    disabled={submitting}
                    className="bg-white text-black font-extrabold uppercase tracking-widest px-8 py-4 rounded hover:bg-yellow-400 transition transform active:scale-95 disabled:opacity-50"
                  >
